...
def justifications_traitees():
    ...
    query = """
        ...
        JOIN eleves e ON a.eleve_id = e.id
        JOIN classes c ON e.classe_id = c.id
        WHERE a.justification_status IN ('acceptee', 'refusee')
    """
    params = []
    ...
    if classe_id:
        ...
        cursor.execute("SELECT id, nom, prenom FROM eleves WHERE classe_id = %s ORDER BY nom, prenom", (classe_id,))
        eleves = cursor.fetchall()
    else:
        eleves = []
    ...
    # Ajouter l'ordre
    query += " ORDER BY a.date DESC, a.creneau"
