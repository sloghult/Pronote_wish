...
@prof_bp.route("/get_eleves_by_classe/<classe>")
@login_required
...
@login_required
def faire_appel(classe_id, date, creneau):
    cursor = get_db().cursor(dictionary=True)
    ...
    """, (session['user_id'], classe_id, classe_id, classe_id))
    if not cursor.fetchone():
        flash("Vous n'avez pas accès à cette classe", "danger")
        return redirect(url_for('prof.appel'))
    
    if request.method == 'POST':
        # Récupérer les élèves absents (cases cochées)
